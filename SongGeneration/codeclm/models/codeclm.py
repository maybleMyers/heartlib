...
class CodecLM:
    ...
    def _apply_img2img_noise_per_channel(
        self,
        tokens: torch.Tensor,
        channel_strengths: tp.List[float]
    ) -> torch.Tensor:
        """Apply per-channel token corruption for img2img generation.

        Args:
            tokens: [B, K, T] discrete audio codes where K=3 (melody, vocal, bgm)
            channel_strengths: [melody_strength, vocal_strength, bgm_strength]
                              Each 0.0-1.0, higher = more corruption

        Returns:
            Corrupted tokens with selective per-channel masking
        """
        assert len(channel_strengths) == 3, "Expected 3 channel strengths (melody, vocal, bgm)"
        corrupted = tokens.clone()

        for ch_idx, strength in enumerate(channel_strengths):
            if strength <= 0.0:
                continue  # No corruption - preserve original tokens
            if strength >= 1.0:
                corrupted[:, ch_idx, :] = -1  # Full corruption
            else:
                # Random masking based on strength
                mask = torch.rand(tokens.shape[0], 1, tokens.shape[2], device=tokens.device) < strength
                corrupted[:, ch_idx:ch_idx+1, :] = torch.where(
                    mask,
                    torch.full_like(corrupted[:, ch_idx:ch_idx+1, :], -1),
                    corrupted[:, ch_idx:ch_idx+1, :]
                )
        return corrupted
    ...
    def generate(self, lyrics: tp.List[str],
        ...
        Args:
            ...
            img2img_mode (str): Mode for img2img generation.
                "uniform" = apply img2img_strength uniformly to all channels
                "vocal_replace" = preserve BGM, fully regenerate vocals from lyrics
                "bgm_replace" = preserve vocals, fully regenerate BGM
        """
        ...
        if img2img_audio is not None:
            ...
            if img2img_mode == "vocal_replace":
                # Preserve BGM and melody, fully regenerate vocals
                channel_strengths = [0.0, 1.0, 0.0]  # [melody, vocal, bgm]
                initial_codes = self._apply_img2img_noise_per_channel(source_codes, channel_strengths)
            elif img2img_mode == "bgm_replace":
                # Preserve vocals and melody, regenerate BGM
                channel_strengths = [0.0, 0.0, 1.0]  # [melody, vocal, bgm]
                initial_codes = self._apply_img2img_noise_per_channel(source_codes, channel_strengths)
            elif img2img_strength < 1.0:
                # Legacy uniform mode - apply same strength to all channels
                initial_codes = self._apply_img2img_noise(source_codes, img2img_strength)
            # else: img2img_strength >= 1.0 and uniform mode = pure generation (no initial codes)
